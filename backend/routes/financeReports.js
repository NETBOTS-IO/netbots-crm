...
router.get('/income-breakdown', auth, async (req, res) => {
  try {
    const incomes = await Income.find().populate('project', 'name').populate('client', 'name');
    ...
    incomes.forEach(i => {
      ...
      // Group by Client
      const clientName = i.client ? i.client.name : 'Unspecified';
...
router.get('/ar-aging', auth, async (req, res) => {
  try {
    const invoices = await Invoice.find({ status: { $ne: 'Paid' } }).populate('client', 'name');
    ...
    invoices.forEach(inv => {
      ...
      const item = {
        ...
        client: inv.client ? inv.client.name : 'Unknown Client',
        amount: inv.amount,
        outstanding,
        dueDate: inv.dueDate
      };
